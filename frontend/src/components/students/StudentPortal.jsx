    ...
    const loadWorkspace = async () => {
      try {
        ...
        const [profileResponse, classmatesResponse, schedulesResponse, announcementsResponse] = await Promise.all([
          ...
          apiRequest('/api/schedules?assignment_status=Assigned&sync=true'),
        ...
        <div className="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
          <section className="rounded-[32px] border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div className="space-y-3">
              {schedules.length === 0 ? (
                <p className="text-sm text-slate-500">No assigned subjects yet for your section.</p>
              ) : (
                  ...
                  >
                    <div className="flex flex-wrap items-start justify-between gap-2">
                      <div>
                        ...
                        <p className="mt-1 text-xs text-slate-500">{schedule.instructor || 'Instructor not assigned yet'}</p>
                      </div>
      ...
      {activeTab === 'schedule' ? (
        <section className="rounded-[32px] border border-slate-200 bg-white p-6 shadow-sm">
          ...
          {groupedSchedule.length === 0 ? (
            <p className="text-sm text-slate-500">No assigned class schedule yet.</p>
          ) : (
                    ...
                    {items.map((schedule) => (
                      <div key={schedule.id} className="rounded-2xl bg-white p-4">
                        ...
                        <div className="mt-2 flex flex-wrap gap-3 text-xs text-slate-500">
                          ...
                          <span>{schedule.instructor || 'Instructor not assigned yet'}</span>
                        </div>
                      </div>
                    ))}
                  </div>
